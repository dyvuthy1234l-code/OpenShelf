        ...
        <table className="w-full text-left text-sm align-middle border-collapse table-fixed">
          <thead>
            <tr className="bg-slate-50/80 border-b border-slate-200/80 text-xs uppercase tracking-wider text-slate-500 font-bold whitespace-nowrap">
              <th className="py-4 px-5 w-[25%]">Member</th>
              <th className="py-4 px-4 w-[27%]">Email</th>
          ...
          >
            {members.map((m) => (
              ...
              >
                ...
                <td className="py-4 px-5 font-bold text-slate-900 whitespace-nowrap min-w-0">
                  <div className="flex items-center gap-3 min-w-0">
                    {m.avatar_url ? (
                      ...
                      <div className="w-9 h-9 rounded-full bg-amber-500 text-slate-950 font-extrabold text-xs flex items-center justify-center shrink-0 shadow-2xs">
                    ...
                    <div className="min-w-0 flex-1">
                      <Link
                        ...
                        className="hover:text-amber-600 transition-colors truncate block font-extrabold text-sm text-slate-900"
                        title={m.name}
                      >
                        {m.name}
                      </Link>
                ...
                {/* Email */}
                <td className="py-4 px-4 text-slate-600 font-medium whitespace-nowrap min-w-0 text-sm">
                ...
                <td className="py-4 px-5 text-right whitespace-nowrap">
                  <div className="flex items-center justify-end">
                    <Link
                      to={`/librarian/members/${m.id}`}
                      className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-800 font-extrabold text-xs rounded-xl transition-colors cursor-pointer shadow-2xs"
                    >
