...
export default function BorrowRequestFilters({
  search,
  onSearchChange,
  status,
  onStatusChange,
  ...
  const statuses = [
    ...
    { value: 'approved', label: '🔵 Approved (Await Pickup)' },
      ...
      <div className="flex items-center gap-2 shrink-0">
        <div className="flex items-center gap-2 flex-1 sm:flex-initial">
          ...
          <select
            ...
            className="w-full sm:w-auto px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 focus:outline-hidden focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500"
          >
            {statuses.map((st) => (
        ...
        {hasActiveFilters && (
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl transition-colors shrink-0"
