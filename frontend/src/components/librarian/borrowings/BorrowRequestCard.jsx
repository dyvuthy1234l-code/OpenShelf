      ...
      <div className="space-y-3">
        ...
        <div className="bg-slate-50 p-3 rounded-2xl border border-slate-200/70 space-y-1 text-xs">
          <div className="flex items-center gap-2 font-bold text-slate-900">
            ...
            <Link to={`/librarian/borrow-requests/${borrowing.id}`} className="hover:text-amber-700 truncate">
        ...
        <div className="flex justify-between items-center text-[11px] text-slate-500">
          <span>Requested: {borrowing.created_at || borrowing.requested_at ? new Date(borrowing.created_at || borrowing.requested_at).toLocaleDateString() : 'N/A'}</span>
          <span className={`font-bold ${isAvailable ? 'text-emerald-700' : 'text-rose-600'}`}>
            {borrowing.book?.available_quantity ?? 0} avail
          </span>
        </div>
      </div>
      ...
      <div className="pt-3 border-t border-slate-100 flex items-center justify-end gap-2 text-xs">
        {borrowing.status === 'pending' && (
          <>
            ...
            <button
              onClick={() => onReject(borrowing)}
              className="px-3 py-1.5 bg-rose-50 hover:bg-rose-100 border border-rose-200 text-rose-700 font-bold text-xs rounded-xl transition-colors"
            >
              Reject
            </button>
          </>
        ...
        <Link
          to={`/librarian/borrow-requests/${borrowing.id}`}
